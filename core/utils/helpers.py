...
from core.utils.constants import  FRAME_ACTIONS_PATH, ROOT_PATH
import shutil
...
def extract_keypoints(results):
    ...
    #face = np.array([[res.x, res.y, res.z] for res in results.face_landmarks.landmark]).flatten() if results.face_landmarks else np.zeros(468*3) #borrar pa optimizar despues
...
def get_keypoints(model, path):
    ...
    '''
    kp_seq = np.array([])
    for img_name in os.listdir(path): #../frames_actions/hola/exanple1..2..3...4/img1...img2....img3.....
...
def count_samples(word_name):
    ...
    if not os.path.isdir(word_path):
        raise FileNotFoundError(f"La palabra '{word_name}' no existe en la ruta especificada: {word_path}")

    # Contar las carpetas dentro de la palabra
    num_folders = sum(1 for item in os.listdir(word_path) if os.path.isdir(os.path.join(word_path, item)))
...
def delete_files(word_name, words_path):
    ...
    if os.path.exists(word_path):
        ...
        print(f"La carpeta o archivo '{word_path}' no existe.")
